...
export class Storage {
	...
	static async getPokemonViewComponents(targetUser: User, pokemonId: number) {
		const pokemons = DATA_SOURCE.getRepository(Pokemon)
		const pokemon = await pokemons.findOne({where: [{id: pokemonId, storedBy: targetUser.id}, {id: pokemonId, heldBy: targetUser.id}]
	})
		if (!pokemon) return {content: `Pokemon #${pokemonId} not found in **${targetUser.displayName}**'s PC`, ephemeral: true}

		const pokemonDetails = await getCachedByIdOrCacheResult(`pokemon:${pokemon.pokeAPIId}`, () => getPokemonDetails(pokemon.pokeAPIId.toString()))
			.catch(error => {
				if (error.response?.status !== 404) {
					console.error(`An error occured during fetch of pokemon's details ${pokemon.id} (#${pokemon.pokeAPIId})`, error)
					return 'error' as const
				}
				return 'unknown' as const
			})
		...
		return {
			...
			components: [
				new ActionRowBuilder<MessageActionRowComponentBuilder>({
					components: [
						new ButtonBuilder().setLabel('Refresh').setEmoji('🔃').setStyle(ButtonStyle.Secondary).setCustomId(`pokemon-view-${pokemon.id}-${targetUser.id}`),
					],
				}),
			],
		}
	...
	async view(
		...
		@SlashOption({name: 'pokemon-id', description: 'ID of the Pokemon to view', required: false, type: ApplicationCommandOptionType.Integer})
